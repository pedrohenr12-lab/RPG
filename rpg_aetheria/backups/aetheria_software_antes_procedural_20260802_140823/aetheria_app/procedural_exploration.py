...
class ProceduralExploration:
    ...
    def _idle_turn(self, opening: str) -> ExplorationTurn:
        ...
        narrative = (
            ...
            f"({self.session.position_x:+d}, {self.session.position_y:+d})."
            f"{objective}{urgency}"
        )
        choices: list[dict[str, Any]] = []
    ...
    def _support_choices(self) -> list[dict[str, Any]]:
        ...
        if self.session.thirst >= 35:
            result.append(self._choice("support:water", "Procurar gelo limpo e derreter água (30 min)", "urgent"))
    ...
    def _travel(self, direction: str) -> ExplorationTurn:
        ...
        encounter = self._roll_interruption(0.075, direction, near_civilization=False)
        if encounter:
    ...
    def _progress_journey(self, pace: str) -> ExplorationTurn:
        ...
        self.state["quiet_blocks"] += 1
        quiet = self._quiet_travel_text("a rota do objetivo", minutes, traveled)
        return self._idle_turn(quiet + march_note + survival_note)
    ...
    def _apply_movement_cost(self, minutes: int, distance: float, pace: str) -> str:
        ...
        energy_cost = {"careful": 2, "normal": 3, "fast": 6}.get(pace, 3)
        ...
        if pace == "fast":
            cold = max(0, cold - 1)
        self.session.temperature = max(-100, self.session.temperature - cold)
    ...
    def _roll_interruption(
        ...
        *,
        ...
        if near_civilization:
            ...
            weights = (25, 18, 18, 14, 15 if fast else 10, 10)
        else:
    ...
    def _quiet_travel_text(self, direction: str, minutes: int, distance: float) -> str:
        ...
        phase_details = DAY_PHASE_DETAILS[self.session.day_phase]
    ...
    def _support(self, action: str) -> ExplorationTurn:
        ...
        if action == "sleep":
            ...
            sheltered = "abrigo_procedural_frostreach" in self.session.flags
            self.session.complete_rest(sleep_minutes)
            if sheltered:
                self.session.change_need("energy", 55)
                self.session.temperature = min(100, self.session.temperature + 12)
                text = "O abrigo sustenta o vento. Você dorme em períodos longos e acorda quando a luz começa a retornar."
            else:
                self.session.change_need("energy", 30)
                self.session.temperature = max(-100, self.session.temperature - 12)
                text = "Sem abrigo adequado, o sono é interrompido pelo frio. Ainda assim, várias horas passam e um novo período do dia começa."
