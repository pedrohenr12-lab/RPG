...
REGION_NPCS = {"frostreach": FROSTREACH_NPCS, "eldorwood": ELDORWOOD_NPCS}
REGION_SETTLEMENTS = {
...
class ProceduralExploration:
    ...
    def _nearest_settlement(self) -> tuple[dict[str, Any] | None, float]:
        ...
        if not settlements or self.session.region_slug != "eldorwood":
    ...
    def _quiet_travel_text(self, direction: str, minutes: int, distance: float) -> str:
        ...
        quiet = ELDORWOOD_QUIET_TRAVEL if self.session.region_slug == "eldorwood" else frost_quiet
        detail = quiet[(self.session.exploration_step + self.state["quiet_blocks"]) % len(quiet)]
        phase_table = ELDORWOOD_PHASE_DETAILS if self.session.region_slug == "eldorwood" else DAY_PHASE_DETAILS
    ...
    def _select_npc(self) -> dict[str, Any]:
        ...
        if self.session.region_slug == "eldorwood":
            ...
            travelers = [
                npc for npc in candidates
                if npc.get("faction") in {"estradas_livres", "arquivo_da_lanterna", "cartografos_da_aurora"}
            ]
