...
from .stonevale_content import STONEVALE_REGION_MAPS
from .blackmarsh_content import BLACKMARSH_REGION_MAPS
from .espinha_content import ESPINHA_REGION_MAPS
...
class StatPanel(QGroupBox):
    def __init__(self):
        ...
        self.tropical_condition = QLabel("")
        self.tropical_condition.setWordWrap(True)
        self.mountain_condition = QLabel("")
        self.mountain_condition.setWordWrap(True)
        self.layout.addWidget(self.clock)
        self.layout.addWidget(self.inventory)
        self.layout.addWidget(self.tropical_condition)
        self.layout.addWidget(self.mountain_condition)
    ...
    def update_session(self, session: PlayerSession) -> None:
        ...
        inventory = ", ".join(session.inventory[-5:]) if session.inventory else "vazio"
        self.inventory.setText(f"Moedas: {session.coins} coroas\nInventário: {inventory}")
        if session.region_slug == "blackmarsh" or any((session.wetness, session.toxicity, session.infection)):
            self.tropical_condition.setText(
                f"Condição tropical: umidade {session.wetness} | "
                f"toxinas {session.toxicity} | infecção {session.infection}"
            )
        else:
            self.tropical_condition.setText("")
        if session.region_slug == "espinha_do_mundo" or any((session.hypoxia, session.vertical_risk)):
            self.mountain_condition.setText(
                f"Condição de altitude: hipóxia {session.hypoxia} | "
                f"risco vertical {session.vertical_risk}"
            )
        else:
            self.mountain_condition.setText("")
...
class GamePage(QWidget):
    ...
    def _choose(self, option: dict) -> None:
        ...
        scripted_combat = option.get("combate")
        if isinstance(scripted_combat, dict):
            event = dict(scripted_combat)
            event.setdefault("kind", "fauna")
            event.setdefault("slug", str(event.get("name") or "combate_roteirizado").casefold().replace(" ", "_"))
            event["_scripted"] = True
            event["_source_scene"] = source_scene_id
            event["_destination_success"] = option.get("destino")
            event["_destination_escape"] = scripted_combat.get("destino_fuga") or source_scene_id
            event["_transition_success"] = option.get("transicao") or "O confronto termina e a consequência abre o próximo trecho da história."
            event["_transition_escape"] = scripted_combat.get("transicao_fuga") or "Você rompe o contato. O perigo permanece no mundo e a história retorna ao ponto de decisão."
            if self.core:
                self.core.record_choice(
                    scene_id=source_scene_id,
                    option_text=str(option.get("texto") or "Enfrentar a ameaça"),
                    destination=None,
                    result_key="combat_started",
                )
            self._start_combat(event)
            return
    ...
    def _condition_met(self, condition: dict | None) -> bool:
        ...
        if condition.get("nao_tem_item") and condition["nao_tem_item"] in self.session.inventory:
            return False
        if condition.get("moedas_minimas") is not None and self.session.coins < int(condition["moedas_minimas"]):
            return False
    ...
    def _apply_effects(self, effects: list[dict] | None, *, skip_time: bool = False) -> None:
        for effect in effects or []:
            ...
            elif kind == "remover_item" and value:
                try:
                    self.session.inventory.remove(str(value))
                except ValueError:
                    pass
            elif kind == "moedas":
                before = self.session.coins
                self.session.coins = max(0, before + int(value or 0))
                self.session.economy_history.append({
                    "day": self.session.day,
                    "hour": self.session.hour,
                    "minute": self.session.minute,
                    "delta": self.session.coins - before,
                    "reason": str(effect.get("motivo") or "transação"),
                    "settlement": effect.get("assentamento"),
                })
            elif kind == "dano":
            ...
            elif kind in {"umidade", "toxicidade", "infeccao", "wetness", "toxicity", "infection"}:
                attribute = {
                    "umidade": "wetness", "toxicidade": "toxicity", "infeccao": "infection",
                }.get(kind, kind)
                current = int(getattr(self.session, attribute))
                setattr(self.session, attribute, max(0, min(100, current + int(value or 0))))
            elif kind in {"hipoxia", "risco_vertical", "hypoxia", "vertical_risk"}:
                attribute = {
                    "hipoxia": "hypoxia", "risco_vertical": "vertical_risk",
                }.get(kind, kind)
                current = int(getattr(self.session, attribute))
                setattr(self.session, attribute, max(0, min(100, current + int(value or 0))))
            elif kind == "xp":
    ...
    def _combat_choice(self, action_id: str) -> None:
        ...
        event = dict(self.combat.state.event)
        combat_summary = self.combat.summary()
        if outcome == "defeat":
        ...
        self.combat = None
        if event.get("_scripted"):
            successful = outcome in {"victory", "captured_enemy", "enemy_fled"}
            destination = event.get("_destination_success" if successful else "_destination_escape")
            transition = event.get("_transition_success" if successful else "_transition_escape")
            scene = self.content.scene(str(destination)) if destination else None
            if scene:
                self.render_scene(scene, combat_summary + "\n\n" + str(transition or ""))
            else:
                self._start_exploration(combat_summary + "\n\n" + str(transition or ""))
            self.stats.update_session(self.session)
            self.session_changed.emit(self.session)
            return
        if self.procedural:
...
class MainWindow(QMainWindow):
        ...
        if result.ok:
            ...
            try:
                ...
                self.game.set_world_catalog(
                    ...
                    + self.repository.exploration_species("stonevale")
                    + self.repository.exploration_species("blackmarsh")
                    + self.repository.exploration_species("espinha_do_mundo")
                )
    ...
    def start_game(self, name: str, race_slug: str, region_slug: str, battle_class_slug: str, profession_slug: str) -> None:
        ...
        elif region_slug == "stonevale":
            stonevale_map = STONEVALE_REGION_MAPS["stonevale"]
            spawn_biome = random.choice(tuple(stonevale_map["spawns"]))
            spawn_position = random.choice(stonevale_map["spawns"][spawn_biome])
            scene_id = {
                "platos_aridos": "00_despertar_stonevale_platos",
                "canions_profundos": "00_despertar_stonevale_canions",
                "vales_ferteis_isolados": "00_despertar_stonevale_vales",
            }[spawn_biome]
        elif region_slug == "blackmarsh":
            blackmarsh_map = BLACKMARSH_REGION_MAPS["blackmarsh"]
            spawn_biome = random.choice(tuple(blackmarsh_map["spawns"]))
            spawn_position = random.choice(blackmarsh_map["spawns"][spawn_biome])
            scene_id = {
                "pantanos_vastos": "00_despertar_blackmarsh_pantanos",
                "mangues_gigantes": "00_despertar_blackmarsh_mangues",
                "ilhas_vegetacao_flutuante": "00_despertar_blackmarsh_ilhas",
            }[spawn_biome]
        elif region_slug == "espinha_do_mundo":
            espinha_map = ESPINHA_REGION_MAPS["espinha_do_mundo"]
            spawn_biome = random.choice(tuple(espinha_map["spawns"]))
            spawn_position = random.choice(espinha_map["spawns"][spawn_biome])
            scene_id = {
                "cordilheira_monumental": "00_despertar_espinha_cordilheira",
                "vales_profundos": "00_despertar_espinha_vales",
                "cavernas_gigantes": "00_despertar_espinha_cavernas",
            }[spawn_biome]
        ...
        if region_slug == "eldorwood":
            ...
            session.flags.add(f"spawn_stonevale_{spawn_biome}")
        elif region_slug == "blackmarsh":
            session.flags.add(f"spawn_blackmarsh_{spawn_biome}")
        elif region_slug == "espinha_do_mundo":
            session.flags.add(f"spawn_espinha_do_mundo_{spawn_biome}")
        save_note = ""
