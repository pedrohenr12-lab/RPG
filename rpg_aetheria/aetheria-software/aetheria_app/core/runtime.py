...
ARKANOR_ROOT_QUEST = "arkanor_o_manuscrito_le_voce"
STONEVALE_ROOT_QUEST = "stonevale_a_voz_antes_do_som"
BLACKMARSH_ROOT_QUEST = "blackmarsh_limiar"
ESPINHA_ROOT_QUEST = "espinha_ancora_profundidade"
...
class PersistentCore:
    ...
    def enter_scene(self, scene_id: str) -> list[dict[str, Any]]:
        ...
        self._arkanor_scene_hook(scene_id, first_visit)
        self._stonevale_scene_hook(scene_id, first_visit)
        self._blackmarsh_scene_hook(scene_id, first_visit)
        self._espinha_scene_hook(scene_id, first_visit)
    ...
    def record_choice(
        ...
        *,
        ...
        self._arkanor_choice_hook(scene_id, destination)
        self._stonevale_choice_hook(scene_id, destination)
        self._blackmarsh_choice_hook(scene_id, destination)
        self._espinha_choice_hook(scene_id, destination)
        self.process_due_events()
        return self.drain_notifications()
    ...
    def _handle_resolved_event(self, event: ScheduledEvent) -> None:
        ...
        elif event.event_type == "arkanor_measure_world_step":
            quest = self.quests.get(ARKANOR_ROOT_QUEST)
            involved = self.world.get("arkanor.ancora_medida.player_involved", False)
            if quest and quest.get("status") not in TERMINAL_QUEST_STATUSES and not involved:
                self.world.set(
                    "arkanor.ancora_medida.world_progress",
                    "fragment_secured_by_conclave",
                    category="quest",
                    source=event.event_id,
                    description=(
                        "Sem a participação do personagem, o Conclave retirou o fragmento da audiência pública, "
                        "enquanto Liora distribuiu cópias incompletas a pesquisadores independentes."
                    ),
                )
                self.quests.set_status(
                    ARKANOR_ROOT_QUEST,
                    "transformed",
                    outcome="conclave_secures_fragment",
                    source=event.event_id,
                )
        elif event.event_type == "stonevale_echo_world_step":
            quest = self.quests.get(STONEVALE_ROOT_QUEST)
            involved = self.world.get("stonevale.ancora_eco.player_involved", False)
            if quest and quest.get("status") not in TERMINAL_QUEST_STATUSES and not involved:
                self.world.set(
                    "stonevale.ancora_eco.world_progress",
                    "mesa_restringe_ecos_e_kharos_busca_hospedeiro",
                    category="quest",
                    source=event.event_id,
                    description=(
                        "Sem a participação do personagem, a Mesa dos Poços restringiu o uso público dos Ecos. "
                        "Erian preservou cópias clandestinas, enquanto Kharos procurou um novo hospedeiro perto de Karsthal."
                    ),
                )
                self.quests.set_status(
                    STONEVALE_ROOT_QUEST,
                    "transformed",
                    outcome="mesa_restricts_echoes",
                    source=event.event_id,
                )
        elif event.event_type == "blackmarsh_limiar_world_step":
            quest = self.quests.get(BLACKMARSH_ROOT_QUEST)
            involved = self.world.get("blackmarsh.limiar.player_involved", False)
            if quest and quest.get("status") not in TERMINAL_QUEST_STATUSES and not involved:
                self.world.set(
                    "blackmarsh.limiar.world_progress",
                    "maeva_expande_sacrificios_e_neris_evacia_sem_registro",
                    category="quest",
                    source=event.event_id,
                    description=(
                        "Sem a participação do personagem, Maeva ampliou as listas que alimentam o selo; "
                        "Neris retirou parte das pessoas sem registro, e a quarentena da febre endureceu."
                    ),
                )
                self.quests.set_status(
                    BLACKMARSH_ROOT_QUEST,
                    "transformed",
                    outcome="sacrifices_expand_and_rescue_network_reacts",
                    source=event.event_id,
                )
        elif event.event_type == "espinha_profundidade_world_step":
            quest = self.quests.get(ESPINHA_ROOT_QUEST)
            involved = self.world.get("espinha.profundidade.player_involved", False)
            if quest and quest.get("status") not in TERMINAL_QUEST_STATUSES and not involved:
                self.world.set(
                    "espinha.profundidade.world_progress",
                    "guildas_expandem_forja_e_amortecedores_falham",
                    category="quest",
                    source=event.event_id,
                    description=(
                        "Sem a participação do personagem, as guildas ampliaram a Forja Profunda para manter empregos e ventilação; "
                        "novos amortecedores cederam, e o Refúgio Velado iniciou uma evacuação clandestina."
                    ),
                )
                self.quests.set_status(
                    ESPINHA_ROOT_QUEST,
                    "transformed",
                    outcome="forges_expand_and_dampers_fail",
                    source=event.event_id,
                )
    ...
    def _arkanor_scene_hook(self, scene_id: str, first_visit: bool) -> None:
        if not first_visit or scene_id != "r3_q01_manuscrito_le_voce":
            return
        self.world.set(
            "arkanor.ancora_medida.player_involved",
            False,
            category="quest",
            source=scene_id,
            visibility="system",
        )
        self.quests.discover(ARKANOR_ROOT_QUEST, rumor=True, source=scene_id)
        self.quests.set_stage(ARKANOR_ROOT_QUEST, "q01", source=scene_id)
        self.scheduler.schedule(
            "arkanor_measure_world_step",
            in_minutes=4 * 24 * 60,
            title="O conflito pelo Manuscrito avançou",
            description=(
                "Quatro dias se passaram. Liora e o Conclave tomaram decisões sobre o fragmento "
                "sem esperar pelo personagem."
            ),
            unique_key="arkanor_measure_world_step",
            condition={"fact": "arkanor.ancora_medida.player_involved", "not_equals": True},
        )
        self.notify(
            "Novo rumor — O manuscrito lê você",
            "A investigação é opcional, mas as instituições de Arkanor continuarão agindo no calendário.",
            kind="quest",
        )

    def _arkanor_choice_hook(self, scene_id: str, destination: str | None) -> None:
        if scene_id != "r3_q01_manuscrito_le_voce":
            return
        if destination and destination.startswith("r3_q"):
            self.world.set(
                "arkanor.ancora_medida.player_involved",
                True,
                category="quest",
            ...
            self.quests.activate(ARKANOR_ROOT_QUEST, stage="q01", source=scene_id)

    def _stonevale_scene_hook(self, scene_id: str, first_visit: bool) -> None:
        if not first_visit or scene_id != "r4_q01_a_resposta_antes_da_pergunta":
            return
        self.world.set(
            "stonevale.ancora_eco.player_involved",
            False,
            category="quest",
            source=scene_id,
            visibility="system",
        )
        self.quests.discover(STONEVALE_ROOT_QUEST, rumor=True, source=scene_id)
        self.quests.set_stage(STONEVALE_ROOT_QUEST, "q01", source=scene_id)
        self.scheduler.schedule(
            "stonevale_echo_world_step",
            in_minutes=5 * 24 * 60,
            title="As vozes de Stonevale mudaram de mãos",
            description=(
                "Cinco dias se passaram. A Mesa dos Poços, os estudiosos dos Ecos e as patrulhas de Karsthal "
                "agiram sem esperar pelo personagem."
            ),
            unique_key="stonevale_echo_world_step",
            condition={"fact": "stonevale.ancora_eco.player_involved", "not_equals": True},
        )
        self.notify(
            "Novo rumor — A voz antes do som",
            "A investigação é opcional. Se você partir, Stonevale continuará decidindo o destino dos Ecos no calendário.",
            kind="quest",
        )

    def _stonevale_choice_hook(self, scene_id: str, destination: str | None) -> None:
        if scene_id != "r4_q01_a_resposta_antes_da_pergunta":
            return
        if destination and destination.startswith("r4_q"):
            self.world.set(
                "stonevale.ancora_eco.player_involved",
                True,
                category="quest",
            ...
            self.quests.activate(STONEVALE_ROOT_QUEST, stage="q01", source=scene_id)

    def _blackmarsh_scene_hook(self, scene_id: str, first_visit: bool) -> None:
        if not first_visit or scene_id != "r5_q01_as_ilhas_formam_uma_porta":
            return
        self.world.set(
            "blackmarsh.limiar.player_involved",
            False,
            category="quest",
            source=scene_id,
            visibility="system",
        )
        self.quests.discover(BLACKMARSH_ROOT_QUEST, rumor=True, source=scene_id)
        self.quests.set_stage(BLACKMARSH_ROOT_QUEST, "q01", source=scene_id)
        self.scheduler.schedule(
            "blackmarsh_limiar_world_step",
            in_minutes=6 * 24 * 60,
            title="O equilíbrio do Limiar mudou",
            description=(
                "Seis dias se passaram. Maeva, Neris, os curadores e as comunidades das duas margens "
                "tomaram decisões sem esperar pelo personagem."
            ),
            unique_key="blackmarsh_limiar_world_step",
            condition={"fact": "blackmarsh.limiar.player_involved", "not_equals": True},
        )
        self.notify(
            "Novo rumor — O Limiar",
            "A investigação é opcional. Se você partir, sacrifícios, resgates e quarentena continuarão mudando Blackmarsh no calendário.",
            kind="quest",
        )

    def _blackmarsh_choice_hook(self, scene_id: str, destination: str | None) -> None:
        if scene_id != "r5_q01_as_ilhas_formam_uma_porta":
            return
        if destination and destination.startswith("r5_q"):
            self.world.set(
                "blackmarsh.limiar.player_involved",
                True,
                category="quest",
            ...
            self.quests.activate(BLACKMARSH_ROOT_QUEST, stage="q01", source=scene_id)

    def _espinha_scene_hook(self, scene_id: str, first_visit: bool) -> None:
        if not first_visit or scene_id != "r6_q01_o_pulso_sob_as_minas":
            return
        self.world.set(
            "espinha.profundidade.player_involved",
            False,
            category="quest",
            source=scene_id,
            visibility="system",
        )
        self.quests.discover(ESPINHA_ROOT_QUEST, rumor=True, source=scene_id)
        self.quests.set_stage(ESPINHA_ROOT_QUEST, "q01", source=scene_id)
        self.scheduler.schedule(
            "espinha_profundidade_world_step",
            in_minutes=7 * 24 * 60,
            title="A carga da montanha mudou",
            description=(
                "Sete dias se passaram. Guildas, mineiros, o Refúgio Velado e os guardas da coluna "
                "tomaram decisões sem esperar pelo personagem."
            ),
            unique_key="espinha_profundidade_world_step",
            condition={"fact": "espinha.profundidade.player_involved", "not_equals": True},
        )
        self.notify(
            "Novo rumor — A Âncora da Profundidade",
            "A investigação é opcional. Se você partir, produção, tremores, o dragão e o Refúgio Velado continuarão no calendário.",
            kind="quest",
        )

    def _espinha_choice_hook(self, scene_id: str, destination: str | None) -> None:
        if scene_id != "r6_q01_o_pulso_sob_as_minas":
            return
        if destination and destination.startswith("r6_q"):
            self.world.set(
                "espinha.profundidade.player_involved",
                True,
                category="quest",
            ...
            self.quests.activate(ESPINHA_ROOT_QUEST, stage="q01", source=scene_id)
