...
from .blackmarsh_content import (
    ARKANOR_BLACKMARSH_BORDER,
    BLACKMARSH_BIOMES,
    BLACKMARSH_NPCS,
    BLACKMARSH_PHASE_DETAILS,
    BLACKMARSH_QUIET_TRAVEL,
    BLACKMARSH_REGION_MAPS,
    BLACKMARSH_ROADS,
    BLACKMARSH_SETTLEMENTS,
    STONEVALE_BLACKMARSH_BORDER,
)
from .espinha_content import (
    ESPINHA_BIOMES,
    ESPINHA_NPCS,
...
BIOMES.update(BLACKMARSH_BIOMES)
BIOMES.update(ESPINHA_BIOMES)

REGION_MAPS.update(ARKANOR_REGION_MAPS)
REGION_MAPS.update(STONEVALE_REGION_MAPS)
REGION_MAPS.update(BLACKMARSH_REGION_MAPS)
REGION_MAPS.update(ESPINHA_REGION_MAPS)
if not any(
...
if not any(
    border.get("id") == STONEVALE_BLACKMARSH_BORDER["id"]
    for border in REGION_MAPS["stonevale"]["borders"]
):
    REGION_MAPS["stonevale"]["borders"].append(STONEVALE_BLACKMARSH_BORDER)
if not any(
    border.get("id") == ARKANOR_BLACKMARSH_BORDER["id"]
    for border in REGION_MAPS["arkanor"]["borders"]
):
    REGION_MAPS["arkanor"]["borders"].append(ARKANOR_BLACKMARSH_BORDER)
if not any(
    border.get("id") == FROSTREACH_ESPINHA_BORDER["id"]
    for border in REGION_MAPS["frostreach"]["borders"]
):
    REGION_MAPS["frostreach"]["borders"].append(FROSTREACH_ESPINHA_BORDER)
if not any(
    border.get("id") == STONEVALE_ESPINHA_BORDER["id"]
    for border in REGION_MAPS["stonevale"]["borders"]
):
    REGION_MAPS["stonevale"]["borders"].append(STONEVALE_ESPINHA_BORDER)
...
REGION_NPCS = {
    ...
    "blackmarsh": BLACKMARSH_NPCS,
    "espinha_do_mundo": ESPINHA_NPCS,
}
REGION_SETTLEMENTS = {
    ...
    "blackmarsh": BLACKMARSH_SETTLEMENTS,
    "espinha_do_mundo": ESPINHA_SETTLEMENTS,
}
REGION_ROADS = {
    "eldorwood": ELDORWOOD_ROADS,
    "arkanor": ARKANOR_ROADS,
    "stonevale": STONEVALE_ROADS,
    "blackmarsh": BLACKMARSH_ROADS,
    "espinha_do_mundo": ESPINHA_ROADS,
}
...
class ProceduralExploration:
    ...
    def _set_initial_biome(self) -> None:
        ...
        if existing in BIOMES:
            ...
            biome = "platos_aridos"
        elif self.session.region_slug == "blackmarsh":
            biome = "pantanos_vastos"
    ...
    def _support_choices(self) -> list[dict[str, Any]]:
        ...
        if self.session.thirst >= 35:
            water_text = (
                ...
                else "Procurar Aquanium, coletar chuva ou filtrar água do pântano (45 min)"
                if self.session.region_slug == "blackmarsh"
    ...
    def _apply_movement_cost(self, minutes: int, distance: float, pace: str) -> str:
        ...
        if self.session.region_slug == "blackmarsh":
            # Chuva, água até os joelhos e suor são perigos persistentes: não
            # desaparecem ao trocar de cena e podem evoluir para infecção.
            wet_gain = max(2, math.ceil(minutes / 25))
            if self.state.get("biome") == "mangues_gigantes":
                wet_gain += 2
            self.session.wetness = min(100, self.session.wetness + wet_gain)
            if self.state.get("biome") == "ilhas_vegetacao_flutuante" and pace == "fast":
                self.session.toxicity = min(100, self.session.toxicity + 2)
        elif self.session.region_slug == "espinha_do_mundo":
            biome_key = self.state.get("biome")
            hypoxia_gain = max(1, math.ceil(minutes / 35))
            if biome_key == "cordilheira_monumental":
                hypoxia_gain += 3
            elif biome_key == "cavernas_gigantes":
                hypoxia_gain += 1
            if self.session.race_slug in {"glacari", "aureli", "ferrari"}:
                hypoxia_gain = max(1, hypoxia_gain - 2)
            self.session.hypoxia = min(100, self.session.hypoxia + hypoxia_gain)
            risk_gain = max(1, math.ceil(distance / 2.5))
            if pace in {"fast", "long"}:
                risk_gain += 3
            if biome_key == "cordilheira_monumental":
                risk_gain += 2
            self.session.vertical_risk = min(100, self.session.vertical_risk + risk_gain)
    ...
    def _quiet_travel_text(self, direction: str, minutes: int, distance: float) -> str:
        ...
        quiet_by_region = {
            ...
            "blackmarsh": BLACKMARSH_QUIET_TRAVEL,
            "espinha_do_mundo": ESPINHA_QUIET_TRAVEL,
        }
        ...
        phase_by_region = {
            ...
            "blackmarsh": BLACKMARSH_PHASE_DETAILS,
            "espinha_do_mundo": ESPINHA_PHASE_DETAILS,
        }
    ...
    def _update_biome_from_position(self) -> str:
        ...
        if self.session.region_slug == "eldorwood":
            ...
            if x >= 700 or y <= -1050:
                biome = "ilhas_vegetacao_flutuante"
            elif x <= -1050:
                biome = "mangues_gigantes"
            else:
                biome = "pantanos_vastos"
        elif self.session.region_slug == "espinha_do_mundo":
            if y >= 900 or x <= -1500:
                biome = "cordilheira_monumental"
            elif y <= -750:
                biome = "cavernas_gigantes"
            else:
                biome = "vales_profundos"
    ...
    def _apply_outcome(self, event: dict[str, Any], action: str, success: bool) -> None:
        if success:
            ...
            if action == "use":
                if event.get("poisonous"):
                    if self.session.region_slug == "blackmarsh":
                        self.session.toxicity = min(100, self.session.toxicity + 12)
                    else:
            ...
            if action == "camp":
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.wetness = max(0, self.session.wetness - 18)
        ...
        if action == "use" or (action == "harvest" and event.get("poisonous")):
            if self.session.region_slug == "blackmarsh":
                self.session.toxicity = min(100, self.session.toxicity + self.rng.randint(10, 18))
            else:
    ...
    def _support(self, action: str) -> ExplorationTurn:
        ...
        if action == "sleep":
            ...
            if sheltered:
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.wetness = max(0, self.session.wetness - 28)
                    text = "O estrado elevado e a cobertura ventilada permitem secar roupa e pele. Você acorda com menos umidade e sem contato direto com a água do pântano."
                elif self.session.region_slug == "espinha_do_mundo":
                    self.session.hypoxia = max(0, self.session.hypoxia - 18)
                    self.session.vertical_risk = max(0, self.session.vertical_risk - 15)
                    self.session.temperature = min(100, self.session.temperature + 10)
                    text = "A ancoragem resiste ao vento e a ventilação permanece aberta. O sono reduz hipóxia e fadiga vertical sem fingir que uma noite cura toda aclimatação."
                else:
                    self.session.temperature = min(100, self.session.temperature + 12)
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.wetness = min(100, self.session.wetness + 16)
                    infection_risk = 8 + self.session.wetness // 12
                    die = self.rng.randint(1, 20)
                    if die <= infection_risk:
                        self.session.infection = min(100, self.session.infection + 10)
                    text = "Sem estrado ou cobertura, água, insetos e raízes em movimento fragmentam o sono. Feridas e pele encharcada ficam expostas à infecção."
                elif self.session.region_slug == "espinha_do_mundo":
                    self.session.hypoxia = min(100, self.session.hypoxia + 10)
                    self.session.vertical_risk = min(100, self.session.vertical_risk + 8)
                    self.session.temperature = max(-100, self.session.temperature - 14)
                    text = "Sem ancoragem e proteção respiratória, frio, vento ou ar estagnado fragmentam o sono. Você acorda mais confuso e menos seguro na vertical."
                else:
        ...
        if action == "water":
            if success:
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.toxicity = max(0, self.session.toxicity - 3)
                    text = "Você recolhe água antes da maré, filtra matéria visível, ferve a amostra e espera o cheiro mudar. O cuidado resolve a sede sem somar outra toxina."
                elif self.session.region_slug == "eldorwood":
                    text = "Você verifica corrente, algas e rastros acima da fonte antes de beber e armazenar água."
                elif self.session.region_slug == "espinha_do_mundo":
                    self.session.hypoxia = max(0, self.session.hypoxia - 2)
                    text = "Você coleta degelo ou goteira, verifica minerais e aquece a água. A pausa ajuda a respiração, mas não substitui aclimatação."
                else:
                    text = "Você separa gelo limpo, derrete aos poucos e bebe sem reduzir ainda mais a temperatura do corpo."
            else:
                if self.session.region_slug == "blackmarsh":
                    self.session.toxicity = min(100, self.session.toxicity + 10)
                    self.session.infection = min(100, self.session.infection + 5)
                    self.session.life = max(0, self.session.life - 1)
                    text = "A transparência engana: a amostra traz parasitas ou resíduo de Aquanium. A sede permanece e o corpo registra toxinas e risco de infecção."
                elif self.session.region_slug == "eldorwood":
            ...
            if success:
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.wetness = max(0, self.session.wetness - 12)
                    text = "Você testa raízes, altura da maré e direção da chuva antes de erguer um estrado ventilado. O ponto permite secar equipamento e dormir fora da água."
                elif self.session.region_slug == "espinha_do_mundo":
                    self.session.hypoxia = max(0, self.session.hypoxia - 4)
                    self.session.vertical_risk = max(0, self.session.vertical_risk - 14)
                    self.session.temperature = min(100, self.session.temperature + 12)
                    text = "Você testa rocha, queda de fragmentos e circulação de ar antes de prender três ancoragens independentes. O abrigo reduz exposição e risco de queda."
                else:
                ...
                if self.session.region_slug == "blackmarsh":
                    self.session.wetness = min(100, self.session.wetness + 8)
                    text = "A turfa desloca o apoio quando o peso muda. Parte do estrado afunda, molha o material e precisa ser abandonada."
                elif self.session.region_slug == "espinha_do_mundo":
                    self.session.vertical_risk = min(100, self.session.vertical_risk + 12)
                    self.session.temperature = max(-100, self.session.temperature - 7)
                    text = "Uma ancoragem entra numa placa superficial e se solta sob carga. Você recupera parte do material, mas perde calor e confiança no apoio."
                else:
    ...
    def _survival_consequences(self) -> str:
        ...
        if self.session.region_slug == "blackmarsh":
            if self.session.wetness >= 85:
                self.session.infection = min(100, self.session.infection + 3)
                notes.append("A pele encharcada aumenta o risco de infecção.")
            if self.session.toxicity >= 70:
                damage = 2 if self.session.toxicity < 90 else 4
                self.session.life = max(0, self.session.life - damage)
                notes.append(f"As toxinas acumuladas causam {damage} de dano.")
            if self.session.infection >= 70:
                self.session.change_need("energy", -5)
                self.session.life = max(0, self.session.life - 1)
                notes.append("A infecção causa febre, dano e perda de energia.")
        elif self.session.region_slug == "espinha_do_mundo":
            if self.session.hypoxia >= 70:
                damage = 1 if self.session.hypoxia < 90 else 3
                self.session.life = max(0, self.session.life - damage)
                self.session.change_need("energy", -5)
                notes.append(f"A hipóxia causa confusão, perda de energia e {damage} de dano.")
            if self.session.vertical_risk >= 85:
                damage = self.rng.randint(1, 4)
                self.session.life = max(0, self.session.life - damage)
                self.session.vertical_risk = max(55, self.session.vertical_risk - 18)
                notes.append(f"Uma falha de apoio causa queda curta e {damage} de dano.")
    ...
    def _urgency_text(self) -> str:
        ...
        if self.session.wetness >= 70:
            notes.append("umidade corporal perigosa")
        if self.session.toxicity >= 60:
            notes.append("intoxicação")
        if self.session.infection >= 60:
            notes.append("infecção em progressão")
        if self.session.hypoxia >= 60:
            notes.append("hipóxia")
        if self.session.vertical_risk >= 70:
            notes.append("risco vertical crítico")
