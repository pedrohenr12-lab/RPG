...
class PlayerSession:
    ...
    coins: int = 25
    economy_history: list[dict[str, Any]] = field(default_factory=list)
    ...
    battle_class_slug: str = "guerreiro"
    battle_class_name: str = "Guerreiro"
    profession_slug: str = "cacador_coletor"
    profession_name: str = "Caçador-Coletor"
    level: int = 1
    class_level: int = 1
    ...
    artifact_state: dict[str, Any] = field(default_factory=lambda: {"charge": 0, "instability": 0})
    ...
    def travel(self, direction: str) -> dict:
        ...
        elif self.region_slug == "espinha_do_mundo":
            self.temperature = max(-100, self.temperature - random.randint(2, 6))
            self.hypoxia = min(100, self.hypoxia + random.randint(2, 5))
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "PlayerSession":
        ...
        cleaned["economy_history"] = list(cleaned.get("economy_history") or ())
        ...
        cleaned["unlocked_skills"] = list(cleaned.get("unlocked_skills") or ())
        cleaned["equipment"] = dict(cleaned.get("equipment") or {})
        cleaned["equipped_artifacts"] = list(cleaned.get("equipped_artifacts") or ())
        cleaned["artifact_state"] = dict(cleaned.get("artifact_state") or {})
        cleaned["combat_state"] = dict(cleaned.get("combat_state") or {})
        cleaned["combat_history"] = list(cleaned.get("combat_history") or ())
        cleaned["companions"] = list(cleaned.get("companions") or ())
        return cls(**cleaned)
